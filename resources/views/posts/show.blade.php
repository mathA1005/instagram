<x-user-layout>
    ...
    <div class="max-w-2xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md">
        ...
        <div class="mb-4">
            <img src="{{ asset($post->image_url) }}" alt="{{ $post->description }}">        </div>

        <p class="text-gray-700" > <a href="{{ route('profile.show', $post->user) }}" class="text-gray-700">  {{ $post->user->name }}</a></p>
        ...
        <div class="flex-col space-y-4">
            @forelse ($post->comments as $comment)
                <div class="flex bg-white rounded-md shadow p-4 space-x-4">
                    <p  class="flex justify-start items-start h-full">  </p>
                        <img src="{{ asset($comment->user->profile_photo) }}" alt="{{ $comment->user->name }}'profile photo" class="w-6 h-6 rounded-full">
                    </a>
